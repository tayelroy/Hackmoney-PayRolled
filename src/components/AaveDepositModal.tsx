...
import { AAVE_BASE_SEPOLIA, AAVE_SEPOLIA } from '@/lib/aave';
...
export function AaveDepositModal({ open, onOpenChange, usdcTokenAddress = SEPOLIA_USDC }: AaveDepositModalProps) {
    ...
    const isSepolia = chainId === AAVE_SEPOLIA.CHAIN_ID;
    const supportedChain = isBaseSepolia || isSepolia;

    // We don't need a useEffect for this, simplified derived state
    const needsChainSwitch = open && !supportedChain;

    const [isSwitching, setIsSwitching] = useState(false);
    const usdcSymbol = isSepolia ? 'USDC (Sepolia)' : 'USDC';

    const handleSwitchChain = async () => {
        setIsSwitching(true);
        try {
            // Default to Base Sepolia if just generic "switch"
    ...
    const handleSwitchToSepolia = async () => {
        setIsSwitching(true);
        try {
            await switchChain({ chainId: AAVE_SEPOLIA.CHAIN_ID });
        } catch (err) {
    ...
    const handleDeposit = async () => {
        ...
        } else if (chainId === AAVE_SEPOLIA.CHAIN_ID) {
            targetTokenAddress = AAVE_SEPOLIA.USDC;
        }
                ...
                <div className="space-y-4 py-4">
                    ...
                    {!needsChainSwitch && (
                        <div className="space-y-2">
                            <Label htmlFor="amount">Amount ({usdcSymbol})</Label>
                            <Input
                                id="amount"
                                type="number"
                                placeholder="10.00"
                            ...
                            <p className="text-xs text-muted-foreground">
                                Directly supply {usdcSymbol} to Aave V3
                            </p>
                        </div>
                    )}
                    ...
                    {status === 'error' && (
                        <Alert variant="destructive">
                            ...
                            <AlertDescription>
                                {error?.includes('2') || error?.includes('execution reverted') ? (
                                    <div className="flex flex-col gap-2">
                                        <span>
                                            <b>Asset Not Supported on Base Sepolia.</b>
                                            <br />
                                            Try switching to <b>Sepolia</b> where Aave supports more assets.
                                        </span>
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={handleSwitchToSepolia}
                                            className="bg-white text-black hover:bg-slate-100 border-slate-300 w-full"
                                        >
                                            Switch to Sepolia
                                        </Button>
                                    </div>
                ...
                <DialogFooter>
                    ...
                    {status !== 'success' && (
                        <Button
                            ...
                            disabled={!amount || parseFloat(amount) <= 0 || status !== 'idle' && status !== 'error' || !supportedChain}
                        >
                            {status === 'approving' ? (
                                <>
                                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
